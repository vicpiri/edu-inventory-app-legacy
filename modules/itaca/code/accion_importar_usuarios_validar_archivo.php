...
if (isset($_GET['file'])){
    ...
    if (busca_columna($datosArchivo,1 ,'tipo') > -1){//Comprobamos si es de personal buscamos 'Tipo'
        require 'accion_importar_usuarios_validar_personal.php';
    }else if (busca_columna($datosArchivo,1 ,'tipus') > -1){//Comprobamos si es de personal buscamos 'tipus'
        require 'accion_importar_usuarios_validar_personal.php';
    }else if (busca_columna($datosArchivo,1 ,'código') > -1){//Comprobamos si es de grupos buscamos 'Código'
        require 'accion_importar_usuarios_validar_grupos.php';
    }else if (busca_columna($datosArchivo,1 ,'codi') > -1){//Comprobamos si es de grupos buscamos 'codi'
        require 'accion_importar_usuarios_validar_grupos.php';
    }else if (busca_columna($datosArchivo,1 ,'nia') > -1){ //Comprobamos si es de alumnado 'NIA'
        require 'accion_importar_usuarios_validar_alumnado.php';
    }else{
