...
//Buscamos si ya existe en la base de datos
    $sql = "SELECT * FROM centros WHERE codigo = $codigoCentro AND NOT id = $idCentro";
    
    $rowsCentros = consultaDB_ALL($sql, $db);
    
    if (count($rowsCentros) > 0){
        $error = 'El código de centro ya existe en la base de datos.';
    }else{
        //Guardamos la información del formulario en la base de datos
        
        $sql = "UPDATE centros SET nombre = '$nombreCentro', codigo = $codigoCentro WHERE id = $idCentro";
        
        consultaDB($sql, $db);
        
        $sql = "SELECT * FROM centros WHERE codigo = $codigoCentro AND nombre LIKE '$nombreCentro' AND id = $idCentro";

        $rowsCentros = consultaDB_ALL($sql, $db);

        if (count($rowsCentros) < 1){
