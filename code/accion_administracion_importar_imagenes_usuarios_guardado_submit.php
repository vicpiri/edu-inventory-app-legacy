...
if (isset($_POST['directorio'])){
    ...
    foreach ((array)$correctos as $imagen) {
        ...
        rename($imagen[1],$baseURL . 'userimages/' . $foto);
        $sql = "UPDATE users SET foto = '$foto' WHERE username = '$imagen[3]'";
        consultaDB($sql, $db);
        
        $sql = "SELECT * FROM users WHERE foto = '$foto'"
                . " AND username = '$imagen[3]'";
        
        $rows = consultaDB_ALL($sql, $db);
        
        if (file_exists($baseURL . 'userimages/' . $foto) && (count($rows) > 0)){
            $count++;
        }
...
?>    
        ...
        <script>
            ...
            $('#subframe').load('code/accion_administracion_importar_imagenes_usuarios.php');
        </script>
